...
async function generatePdfFiles(pluginOptions, siteConfig) {
    ...
    for (const versionInfo of versionInfos) {
        ...
        const sideBars = sidebars_1.loadSidebars(versionInfo.sidebarFile);
...
function pickHtmlArticlesRecursive(sideBarItem, parentTitles, version, rootDocUrl, rootDocId, htmlDir, siteConfig) {
...
async function createPdfFilesRecursive(sideBarItem, parentTitles, documentVersion, pluginOptions, siteConfig, buildDir, browser, siteAddress) {
    let articles = [];
    let documentTitle = '';
    let pdfFilename = '';
    switch (sideBarItem.type) {
    ...
    if (articles.length > 0) {
        await createPdfFromArticles(documentTitle, documentVersion, pdfFilename, articles, pluginOptions, siteConfig, buildDir, browser, siteAddress);
    }
    return articles;
}
...
const getPageWithFixedToc = (footerRegEx, tocList, pdfContent, htmlContent) => {
    ...
    tocList.forEach(e => {
        for (; pageIndex < pdfPages.length; pageIndex++) {
            const regex1 = pdfHeaderRegex[0](e.text);
            const regex2 = pdfHeaderRegex[1](e.text);
            const regex3 = pdfHeaderRegex[2](e.text);
            let page = pdfPages[pageIndex];
            if (regex1.test(page) || regex2.test(page) || regex3.test(page)) {
                htmlContent = htmlContent.replace('<span class="pageNumber">_</span>', `<span class="pageNumber">${pageIndex}</span>`);
                break;
            }
        }
    });
